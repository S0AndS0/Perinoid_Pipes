			...
			case "${Var_save_encryption_yn}" in
				y|Y|yes|Yes)
					...
					if [ -f "${_mapped_array}" ]; then
						...
						Var_star_date="$(date -u +%s)"
						#${Var_cat_exec_path} <<<"${_mapped_array}" | $(which gpg-zip) --encrypt --output ${Var_parsing_bulk_out_dir}/${Var_star_date}_dir${Var_bulk_output_suffix} --gpg-args  -r ${Var_gpg_recipient}
						...
						tar cf "${_mapped_array}" | $(which gpg) --always-trust --armor --batch --encrypt --recipient ${Var_gpg_recipient} --output ${Var_parsing_bulk_out_dir}/${Var_star_date}_dir.tar.gpg
