...
cat <<<"${_test_string}" | gpg ${Var_gnupg_encrypt_options} ${Var_test_gpg_location} >> ${Var_test_gpg_location}
...
if [ -f "${Var_test_gpg_location}" ]; then
	cat  ${Var_pass_location} | gpg ${Var_gnupg_decrypt_options} ${Var_test_gpg_location} >> ${Var_test_raw_location}
