			...
			case "${Var_save_encryption_yn}" in
				y|Y|yes|Yes)
					...
					if [ -f "${_mapped_array}" ]; then
						...
						Var_star_date="$(date -u +%s)"
						gpg-zip --armor --batch --encrypt --recipient ${Var_gpg_recipient} --output ${Var_parsing_bulk_out_dir}/${Var_star_date}_${_mapped_array##*/}${Var_bulk_output_suffix} ${_mapped_array}
						Func_messages "# Encryption command [gpg-zip --armor --batch --encrypt --recipient ${Var_gpg_recipient} --output ${Var_parsing_bulk_out_dir}/${Var_star_date}_${_mapped_array##*/}${Var_bulk_output_suffix} ${_mapped_array}]" '2' '3'
						#Func_messages "# Encryption command [${Var_tar_exec_path} -c \${_mapped_array} | ${Var_parsing_command} >> \"${Var_parsing_bulk_out_dir}/\${Var_star_date}_\${_mapped_array##*/}.tgz${Var_bulk_output_suffix}\"]" '2' '3'
