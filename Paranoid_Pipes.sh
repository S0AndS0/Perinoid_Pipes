			...
			case "${Var_save_encryption_yn}" in
				y|Y|yes|Yes)
					...
					if [ -f "${_mapped_array}" ]; then
						...
						Var_star_date="$(date -u +%s)"
						gpg-zip --encrypt --output ${Var_parsing_bulk_out_dir}/${Var_star_date}_${_mapped_array##*/}${Var_bulk_output_suffix} --gpg-args --recipient ${Var_gpg_recipient} ${_mapped_array}
