			...
			case "${Var_save_encryption_yn}" in
				y|Y|yes|Yes)
					...
					if [ -f "${_mapped_array}" ]; then
						...
						Var_star_date="$(date -u +%s)"
						${Var_cat_exec_path} <<<"${_mapped_array}" | gpg-zip --output ${Var_parsing_bulk_out_dir}/${Var_star_date}_dir${Var_bulk_output_suffix} --recipient ${Var_gpg_recipient} --encrypt
						...
						#tar -cvz "${_mapped_array}" | ${Var_parsing_command} > ${Var_parsing_bulk_out_dir}/${Var_star_date}_dir.tar.gz.gpg
