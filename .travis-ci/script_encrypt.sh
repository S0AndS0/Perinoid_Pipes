...
Func_run_sanely "echo ${_pass_phrase} > ${Var_pass_location}" "${USER}"
...
if [ -e "${Var_install_path}/${Var_install_name}" ]; then
	Func_run_sanely "${Var_install_path}/${Var_install_name} ${Arr_encrypt_opts[*]}" "${USER}"
...
if [ -p "${Var_encrypt_pipe_location}" ]; then
	...
	if [ -r "${Var_encrypted_location}" ]; then
		...
		Func_run_sanely "gpg --batch --yes --decrypt ${Var_encrypted_location} --passphrase-file ${Var_pass_location}" "${USER}"
	else
