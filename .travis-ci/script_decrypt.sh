...
Func_run_sanely "chmod 111 \"${Var_install_path}/${Var_helper_name}\"" "0"
...
if [ -e "${Var_install_name}" ]; then
	...
	echo "${Var_script_name} could not execute: ${Var_install_name}"
fi
## Run decryption with the aid of helper script
if [ -e "${Var_helper_name}" ]; then
	Func_run_sanely "${Var_helper_name} \"${Var_encrypted_location}\" \"${Var_decrypt_pipe_location}\"" "${USER}"
else
	echo "${Var_script_name} could not execute: ${Var_helper_name}"
fi
