...
#_test_decryption_opts="--always-trust --passphrase-fd 3 3< <(cat "${Var_pass_location}") --decrypt"
...
cat <<<"${_test_string}" | tee exec 3<${Var_pass_location}; gpg ${_test_encryption_opts} >> ${Var_test_gpg_location}
_exit_status=$?
