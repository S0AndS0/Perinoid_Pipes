...
- Acceptable values: `'/tmp/test_pipe_listener.sh'` Path and file name to save script copy under
...
- Acceptable values: `'100'` or `'110'` Executable permissions only are sufficient.
...
- Regex restrictions: `a-zA-Z0-9_@,.:~\/`
- Acceptable values: `<user>`:`<group>` Allowed to read named pipe and execute script copy.
...
- Default value: `yes`
- Regex restrictions: `a-zA-Z`
- Acceptable values: 'Yes' or 'No' Enable or disable running named pipe in background.
...
- Default value: `0`
- Regex restrictions: `0-9`
- Acceptable values: `'0'` - `'6'` Lower the number to log less of main script run time.
...
- Regex restrictions: `a-zA-Z0-9_@,.:~\/`
- Acceptable values: `'/var/log/named_pipe_writer.log'` Path and file name for the main script's logs to be written out to.
...
- Default value: `600`
- Regex restrictions: `0-9`
- Acceptable values: `'600'` or `'660'` Read+write permissions are great for debugging.
...
- Default value: `${USER}:${USER}`
- Regex restrictions: `a-zA-Z0-9_@,.:~\/`
- Acceptable values: `user`:`group` Allowed to read and write to main script's log file.
...
- Default value: `yes`
- Regex restrictions: `a-zA-Z`
- Acceptable values: `'Yes'` or `'No'` Enable or disable persistent logs between main script runs.
...
- Regex restrictions: `a-zA-Z0-9_@,.:~\/`
- Acceptable values: `'/tmp/named_test.pipe'` Path and file name to make new named pipe to listen on.
...
- Default value: `600`
- Regex restrictions: `0-9`
- Acceptable values: `'460'` or `'640'` Read/Write permissions separated by owner/group are a good idea.
...
- Default value: `${USER}:${USER}`
- Regex restrictions: `a-zA-Z0-9_@,.:~\/`
- Acceptable values: `<user>`:`<group>` Allowed to read and write to named pipe file.
...
- Default value: `quit`
- Regex restrictions: `a-zA-Z0-9_@,.:~\/`
- Acceptable values: `'quit'` or `"$(base64 /dev/urandom | tr -cd 'a-zA-Z0-9' | head -c32)"` Non-space separated string that will cause listening loops to exit/break on.
...
- Acceptable values: `'rm /tmp/named_test.pipe'` Command to run when named pipe listener reads above string.
...
- Default value: `no`
- Regex restrictions: `a-zA-Z`
- Acceptable values: `'Yes'` or `'No'` Enable or disable reading named pipe input for comments.
...
- Default value: `\#*`
- Regex restrictions: `\#*`
- Acceptable values: `"\#*"` or `"\#*|\;*"` A anonymous pipe (`|`) separated list of known line commenting characters.
...
- Regex restrictions: null
- Acceptable values: `[^a-zA-Z0-9 !#%&:;$\/\^\-\"\(\)\{\}\\]` Allowed characters in lines not preceded by known comment
...
- Regex restrictions: `a-zA-Z0-9_@,.:~\/`
- Acceptable values: `'email_name@email_domain.suffix'` Email address or GPG public key ID to encrypt lines or known file types to.
...
- Default value: `yes`
- Regex restrictions: `a-zA-Z`
- Acceptable values: `yes` or `no` Enable or disable writing parsed output options and actions.
...
- Default value: `yes`
- Regex restrictions: `a-zA-Z`
- Acceptable values: `yes` or `no` Enable or disable log rotation options and actions.
...
- Default value: `4096`
- Regex restrictions: `0-9`
- Acceptable values: `'4096'` or `'8388608'` Output file max size (in bites) before log rotation actions are used.
...
- Default value: `100`
- Regex restrictions: `0-9`
- Acceptable values: `'10'` or `'100000'` Number of output file writes till above file size max vs real file size be checked.
...
- Regex restrictions: `a-zA-Z0-9_@,.:~\/`
- Acceptable values: `'compress-encrypt,remove-old'` or `'encrypted-email,remove-old'` List of actions, separated by commas `,` to take when output file's size and write count reaches above values.
...
- Default value: `user@host.domain`
- Regex restrictions: `a-zA-Z0-9_@,.:~\/`
- Acceptable values: `'admin_name@admin_domain.suffix'` Email address of GPG public key ID to re-encrypt and or send compressed output files to.
...
- Acceptable values: Disabled to avoid errors during user input parsing when spaces are present in values.
...
- Regex restrictions: `a-zA-Z0-9_@,.:~\/`
- Acceptable values: `'/tmp/encrypted_files'` Directory path to save recognized files to. Note these files are not rotated but maybe appended to if not careful.
...
- Default value: `.gpg`
- Regex restrictions: null
- Acceptable values: `'.gpg'` or `'.log'` File suffix to append to bulk encrypted files. Note if decrypting then unset to have previously encrypted file suffixes restored.
...
- Default value: `no`
- Regex restrictions: `a-zA-Z`
- Acceptable values: `yes` or `no` default `no`. Used to control if following two options are considered as options for modifying read data.
...
- Regex restrictions: `a-zA-Z0-9_@,.:~\/`
- Acceptable values: `32` or another integer (whole number) default `adaptive` which assumes the same length as line being read through loop.
...
- Default value: `above`
- Regex restrictions: `a-zA-Z0-9_@,.:~\/`
- Acceptable values: Order applied within loop; `append`, `prepend`, `above`, `bellow`
...
- Default value: null
- Regex restrictions: `a-zA-Z0-9_@,.:~\/`
- Acceptable values: File to source for variables defined in previous table. Or file to save values to, see next two options bellow.
...
- Default value: `no`
- Regex restrictions: null
- Acceptable values: `yes` or `no` Enable or disable saving options file to `--source-var-file`'s path
...
- Default value: `no`
- Regex restrictions: null
- Acceptable values: `yes` or `no` Enable or disable saving variables file to `--source-var-file`'s path
...
- Default value: null
- Regex restrictions: null
- Acceptable values: `null` Prints and exits with `0` script's current license and license of scripts that this project writes.
...
- Acceptable values: `null` Attempts to search for `-h=value` within host system's help documentation and within main script's detailed documentation.
...
- Default value: null
- Regex restrictions: null
- Acceptable values: `null` Writes any unrecognized arguments as lines or words that should be written to named pipe if available.
